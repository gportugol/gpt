...
int create_server_socket(int port) {
  ...
  if (-1 ==
      setsockopt(
          ...
          sizeof(
              on))) { // see
                      // http://www-128.ibm.com/developerworks/linux/library/l-sockpit/
  ...
  while (1) {
    ...
    if (received < 0) {
      ...
      printf("total pack: %d, data received: \n>>>\n%s\n<<<\n", received,
             buffer + 1 + (strlen(buffer) * sizeof(char)));
    }
    printf("\n\n");

    if (send(clientsock, cmd_step_into,
             sizeof(char) * (strlen(cmd_step_into) + 1), 0) < 0) { // error
